...
{
	public sealed partial class MessageThreadUserControl : IMessageThreadView
	{
		...
		public MessageThreadUserControl()
		{
			...
			_messagesViewModel.MessageThreadView = this;
		...
		public async Task UpdateSendMessageStateFromBackgroundThread(Message message)
		{
			await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
			{
				if (MessageThread.Items == null)
				...
				foreach (var item in MessageThread.Items)
				{
					...
					if (bubble.MessageID != message.Id)
						continue;
					if (bubble.TextBlockSendMessageState != null)
					{
						bubble.TextBlockSendMessageState.Text = message.SendMessageState.ToString();
						return;
					}
					Debug.WriteLine("Wrong bubble side, wrong contact!");
					return;
				}
			});
