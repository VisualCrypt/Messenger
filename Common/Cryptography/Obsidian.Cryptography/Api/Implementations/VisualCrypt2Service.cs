...
namespace Obsidian.Cryptography.Api.Implementations
{
    public class VisualCrypt2Service : IVisualCrypt2Service
    {
        ...
        public void Init(IPlatform platform, string name = "VisualCrypt2Service Default Instance")
        {
            ...
            SymmetricKeyRepository = new SymmetricKeyRepository();
        }
            ...
            Compressed compressed)
        {
            ...
            VisualCryptAPI2Internal.IVCache ivCache = roundsExponent.Value == RoundsExponent.DontMakeRounds
                ? null
                : _internal.CreateIVTable(iv, roundsExponent.Value);

            var cipherV2 = new CipherV2 { RoundsExponent = roundsExponent, IV16 = iv };
            _internal.AESEncryptRandomKeyWithInputDerivedKey(inputDerivedKey, randomKey, cipherV2, ivCache, context);
            if (_log)
            {
            ...
            _internal.AESEncryptMessageWithRandomKey(paddedData, randomKey, cipherV2, ivCache, context);
            if (_log)
            {
            ...
            _internal.AESEncryptMACWithRandomKey(cipherV2, mac, randomKey, ivCache, context);
            if (_log)
            {
        ...
        public Response<CipherV2> BinaryEncrypt(Clearbytes clearBytes, KeyMaterial64 keyMaterial64, RoundsExponent roundsExponent, LongRunningOperationContext context)
        {
            ...
            try
            {
                ...
                Compressed compressed = _internal.CompressBytes(clearBytes);
        ...
        MAC16 CreateMAC(CipherV2 cipherV2, LongRunningOperationContext context)
        {
            ...
            {
                var sha256 = _platform.ComputeSHA256(securables);
        ...
        InputDerivedKey32 CreateDerivedKeyWithSHA256(IV16 iv, KeyMaterial64 keyMaterial64)
        {
            ...
            var derivedKey = _platform.ComputeSHA256(keyMaterial);
        ...
        public Response<Clearbytes> BinaryDecrypt(CipherV2 cipherV2, KeyMaterial64 keyMaterial64, LongRunningOperationContext context)
        {
            ...
            try
            {
                ...
                Clearbytes cleartext = _internal.DecompressBytes(compressed);
        ...
        Compressed DecryptCommon(CipherV2 cipherV2, KeyMaterial64 keyMaterial64, LongRunningOperationContext context)
        {
            ...
            VisualCryptAPI2Internal.IVCache ivCache = cipherV2.RoundsExponent.Value == RoundsExponent.DontMakeRounds
                ? null
                : _internal.CreateIVTable(cipherV2.IV16, cipherV2.RoundsExponent.Value);

            RandomKey32 randomKey = _internal.AESDecryptRandomKeyWithPasswordDerivedKey(cipherV2, inputDerivedKey, ivCache, context);

            MAC16 decryptedMAC = _internal.AESDecryptMAC(cipherV2, randomKey, ivCache, context);
            ...
            PaddedData paddedData = _internal.AESDecryptMessage(cipherV2, cipherV2.IV16, randomKey, ivCache, context);
        ...
        public Response<QualifiedRandom> GetRandom(int randomLenght)
        {
            var response = new Response<QualifiedRandom>();
            try
            {
                EnsurePlatform();
                response.Result = new QualifiedRandom { X = _platform.GenerateRandomBytes(randomLenght) };
                response.SetSuccess();
            }
            catch (Exception e)
            {
        ...
        public Response<ECKeyPair> GenerateECKeyPair()
        {
            var response = new Response<ECKeyPair>();
            try
            {
                var randomResponse = GetRandom(32);
        ...
        public byte[] CalculateAndHashSharedSecret(byte[] privateKey, byte[] publicKey)
        {
            ...
            var sharedSecretHash = _platform.ComputeSHA256(sharedSecret);
            return sharedSecretHash;
        }

        public byte[] DefaultEncrypt(byte[] plaintextBytes, KeyMaterial64 nonDefaultKey = null)
        {
            var key = nonDefaultKey ?? SymmetricKeyRepository.GetPasswordHash();
            var binaryEncryptResponse = BinaryEncrypt(new Clearbytes(plaintextBytes), key, new RoundsExponent(RoundsExponent.DontMakeRounds), null);
            return !binaryEncryptResponse.IsSuccess
                ? null
                : VisualCrypt2Formatter.CreateBinary(binaryEncryptResponse.Result);
        }

        public byte[] DefaultDecrypt(byte[] cipherTextBytes, KeyMaterial64 nonDefaultKey = null)
        {
            var key = nonDefaultKey ?? SymmetricKeyRepository.GetPasswordHash();
            CipherV2 cipherV2 = VisualCrypt2Formatter.DissectVisualCryptBytes(cipherTextBytes, null);
            var binaryDecryptResponse = BinaryDecrypt(cipherV2, key, null);
